...
const STORAGE_KEY = 'traffic-flow-gridlock-save-v1';
...
function defaultSave(): PlayerSave {
  ...
  for (let i = 1; i <= TOTAL_LEVELS; i++) levels[i] = defaultLevel(i);
  return {
    coins: 250,
    gems: 15,
    levels,
...
export function recordLevelResult(
  ...
  const prev = save.levels[levelId] ?? defaultLevel(levelId);
  ...
  if (levelId < TOTAL_LEVELS) {
    const unlock = levels[levelId + 1] ?? defaultLevel(levelId + 1);
  ...
  if (levelId >= 10 && !achievements.traffic_rookie) achievements.traffic_rookie = true;
  if (stars === 3 && !achievements.perfect_driver) achievements.perfect_driver = true;
  if (levelId >= 50 && !achievements.flow_master) achievements.flow_master = true;
